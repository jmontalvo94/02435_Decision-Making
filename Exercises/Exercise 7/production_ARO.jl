...
@variable(model_production, 0<=delta[p in P]) #Placeholder variable for missing demand

#Minimize production cost
@objective(model_production, Min, sum(c_m[m]*y[m] for m in M) + sum(c_p[p]*x[p,m] for p in P for m in M) + sum(Phi*delta[p] for p in P))

#Fulfill demand
@constraint(model_production, demand[p in P], sum(x[p,m] for m in M) + delta[p in P] >= d[p])

#Production time
@constraint(model_production, time_con[m in M], sum(t_bar[p]*x[p,m] for p in P) <= t_m[m]*y[m])
