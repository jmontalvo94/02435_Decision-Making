...
@variable(model_production, 0<=lambda[ m in M]) #Dual variable for budget of uncertainty
@variable(model_production, 0<=mu[p in P, m in M]) #Dual variable for budget of uncertainty
...
#Production time
@constraint(model_production, time_con[m in M], sum(t_bar[p]*x[p,m] for p in P) + Gamma[m]*lambda[m] + sum(mu[p,m] for p in P) <= t_m[m]*y[m])

#Dual constraint
@constraint(model_production, dual_con[m in M, p in P], lambda[m]+mu[p,m] >= t_dev[p]*x[p,m])
